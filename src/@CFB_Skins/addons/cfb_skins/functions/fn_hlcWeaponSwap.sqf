...
{
	if (isNil "CFB_HLC_WeaponSwapData") then
	{
		CFB_HLC_WeaponSwapData =
			[
				[
...
if ((count _weaponSwapData == 0) && isClass(configFile >> "CfgPatches" >> "rhs_weapons")) then
{
	...
	_weaponSwapData = CFB_RHS_WeaponSwapData;
};
...
{
	if (isNil "CFB_CUP_WeaponSwapData") then
	{
		CFB_CUP_WeaponSwapData =
			[
				[
					["hgun_PDW2000_F"],
					[
	...
	_weaponSwapData = CFB_CUP_WeaponSwapData;
};
